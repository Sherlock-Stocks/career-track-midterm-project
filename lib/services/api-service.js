...
const getPortfolio = async(userInfo) => {
  ...
  const stockHistory = portfolio.map(stock => {
    ...
    return async(req, res, next) => {
      await fetch(`https://api.twelvedata.com/time_series?start_date=${startDate}&end_date=${endDate}&symbol=${ticker}&interval=${frequency}&apikey=${process.env.API_KEY}`)
        .then(res => res.json())
